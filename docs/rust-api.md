...
| `widen_degenerate_form_bboxes_mem(bytes)` | Rust only (not in the Python, Node.js or WebAssembly bindings). The document re-serialized with the zero-area `/BBox` of its Form XObjects widened, for rendering elsewhere; `None` when no form needs it and for an encrypted document, whose protection a plain serialization would drop |
