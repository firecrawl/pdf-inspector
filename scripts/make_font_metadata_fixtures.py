...
that its notice accompany copies of the fonts, so the script writes it to
``tests/fixtures/FONT_LICENSES.md`` and names it in the PDF's document
...
def build_pdf(programs) -> bytes:
    ...
    info = writer.add(
        (
            ...
            " /Subject (Synthetic fixture: text set in renamed TrueType subsets of the"
            " DejaVu fonts 2.37, redistributed under the Bitstream Vera Fonts licence;"
            " see tests/fixtures/FONT_LICENSES.md)"
...
def fetch_release(cache_dir: Path) -> Path:
    ...
    if digest != DEJAVU_SHA256:
        raise SystemExit(f"{archive}: SHA-256 {digest} does not match the pinned {DEJAVU_SHA256}")
